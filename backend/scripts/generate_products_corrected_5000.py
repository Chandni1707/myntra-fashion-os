...
OUTPUT_JSON = "product_catalog_5000.json"
OUTPUT_CSV = "product_catalog_5000.csv"
...
def create_product(row):
    ...
    return {
        ...
        "gender": row.gender,
        "price": price,
        "colors": extract_colors(str(row.name)),
        "styles": style_list,
...
# ==========================================================
# GENERATE ALL VALID UNIQUE CANDIDATES
# ==========================================================

print("\nGenerating valid fashion candidates...")
random.seed(42)
all_candidates = []
seen_keys = set()
...
for row in df.itertuples(index=False):
    ...
    if product is None:
        continue
    key = (product["name"].lower(), product["image_url"])
    if key in seen_keys:
        continue
    seen_keys.add(key)
    all_candidates.append(product)

print(f"Valid unique fashion candidates: {len(all_candidates)}")

# ==========================================================
# GROUP AND SHUFFLE BY CATEGORY
# ==========================================================

category_pools = {category: [] for category in CATEGORY_LIMITS}
for product in all_candidates:
    category_pools[product["category"]].append(product)
...
for category, limit in CATEGORY_LIMITS.items():
    for product in category_pools[category][:limit]:
        key = (product["name"].lower(), product["image_url"])
...
print(f"\nProducts after balanced first pass: {len(products)}")
...
# Clean stable product IDs after final selection
for i, product in enumerate(products, start=1):
    product["product_id"] = f"P{i:05d}"
...
category_summary = {}
for p in products:
    category_summary[p["category"]] = category_summary.get(p["category"], 0) + 1
